...
2. Adjust secrets (e.g. `NEXTAUTH_SECRET`, `NEXT_PRIVATE_DOCUMENT_PASSWORD_KEY`) and set the public URLs that end users will visit.
3. For the bundled MinIO/Compose stack, switch storage to S3 mode and align the credentials:
   ```dotenv
...
4. Review the optional provider settings. Leaving them blank disables the integration gracefully (Slack, Hanko passkeys, Upstash Redis, QStash, Resend, Trigger.dev, OAuth providers, etc.).
5. Launch the stack:
  ```bash
...
- **Optional integrations:** Provide API keys only for the services you need (Resend, Slack, Upstash Redis, QStash, Trigger.dev, OAuth providers, Hanko). When unset, Papermark falls back to safe no-op implementations suitable for single-instance deployments.
